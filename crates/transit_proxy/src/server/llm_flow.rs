...
pub(super) fn prepare_llm_exchange(
    ...
    if dialect != LlmDialect::OpenAi {
        ...
        let (url, mut translated) = match dialect {
            ...
            LlmDialect::OpenAi | LlmDialect::Codex | LlmDialect::Antigravity => {
                unreachable!("dialect is handled separately")
            }
        };
...
pub(super) fn prepare_oauth_exchange(
    ...
    if account.provider() == "antigravity" {
        if context.path != llm::OPENAI_CHAT_COMPLETIONS_PATH {
            return Err(bad("Antigravity OAuth supports /v1/chat/completions".into()));
        }
        let project = account.document["project_id"]
            .as_str()
            .filter(|value| !value.trim().is_empty())
            .ok_or_else(|| bad("Antigravity OAuth account has no project_id".into()))?;
        let effective_model = backend.rewrite_model(model).unwrap_or(model);
        let streaming = llm::is_streaming_request(&value);
        let mut random = [0_u8; 16];
        SystemRandom::new()
            .fill(&mut random)
            .map_err(|_| bad("Cannot generate Antigravity request ID".into()))?;
        ...
        let translated = llm::antigravity_request(&value, effective_model, project, &request_id);
        let base = match &backend.kind {
            transit_core::BackendKind::Llm {
                endpoint: Some(endpoint),
                ..
            } => endpoint.as_str(),
            _ => "https://daily-cloudcode-pa.googleapis.com",
        };
        let uri = llm::antigravity_generate_url(base, streaming)
            .parse()
            .map_err(|_| bad("Invalid Antigravity endpoint".into()))?;
        return Ok(LlmExchange {
            uri,
            body: Bytes::from(translated.to_string()),
            dialect: LlmDialect::Antigravity,
            streaming,
            body_rewritten: true,
...
pub(super) async fn finalize_llm_response(
    ...
    match exchange.dialect {
        ...
        LlmDialect::Anthropic | LlmDialect::Gemini | LlmDialect::Antigravity => {
            if parts.status.is_success() && exchange.streaming && is_event_stream(&parts.headers) {
                ...
                let translated = match exchange.dialect {
                    ...
                    LlmDialect::Antigravity => {
                        llm::transcode_antigravity_stream(body, exchange.model, sink)
                    }
            ...
            let translated = if parts.status.is_success() {
                let (translated, usage) = match exchange.dialect {
                    ...
                    LlmDialect::Antigravity => llm::openai_from_gemini_response(
