...
pub fn antigravity_generate_url(base: &str, stream: bool) -> String {
    let base = base.trim_end_matches('/');
    if stream {
...
pub fn antigravity_request(body: &Value, model: &str, project: &str, request_id: &str) -> Value {
    let mut request = gemini_request(body);
    request["sessionId"] = Value::String(request_id.to_string());
    json!({
        "project": project,
        "model": model,
...
impl GeminiStream {
    fn new(model: String, sink: UsageSink, wrapped: bool) -> Self {
        Self {
            lines: SseLines::default(),
            model,
            created: now_epoch_seconds(),
...
impl Transcode for GeminiStream {
    fn push(&mut self, chunk: &[u8]) -> Vec<u8> {
        ...
        for line in self.lines.push(chunk) {
            ...
            let event = if self.wrapped {
                envelope.get("response").unwrap_or(&envelope)
            } else {
                &envelope
            };
...
pub fn transcode_gemini_stream(upstream: Body, model: String, sink: UsageSink) -> Body {
    wrap_body(upstream, GeminiStream::new(model, sink, false))
}

pub fn transcode_antigravity_stream(upstream: Body, model: String, sink: UsageSink) -> Body {
    wrap_body(upstream, GeminiStream::new(model, sink, true))
}
