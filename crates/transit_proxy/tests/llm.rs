...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn antigravity_oauth_wraps_gemini_requests_and_responses() {
    let upstream = spawn_app(Router::new().route(
        ...
        post(|headers: HeaderMap, Json(value): Json<Value>| async move {
            ...
            {
                return error_response(StatusCode::BAD_REQUEST, "Antigravity user-agent missing");
            }
            if value["project"] != "project-123"
                || value["model"] != "gemini-test"
                || value["userAgent"] != "antigravity"
                || value["request"]["contents"][0]["parts"][0]["text"] != "hi"
            {
                return error_response(StatusCode::BAD_REQUEST, "Antigravity envelope mismatch");
            }
            json_response(json!({"response":{
                "responseId":"response-123",
                "candidates":[{"content":{"parts":[{"text":"hello from antigravity"}]},"finishReason":"STOP"}],
    ...
    let proxy = spawn_proxy(llm_config(
        provider("google", ProviderKind::Gemini, upstream.addr, ""),
        vec![backend],
        vec![],
    ))
    .await;
    ...
    proxy
        ...
        .save(transit_proxy::OAuthAccount {
            ...
            document: json!({"type":"antigravity","access_token":"antigravity-token","project_id":"project-123"}),
            models: vec![],
            revision: 0,
        })
        .await
