                ...
                models.is_empty()
                    || model
                        .map(|model| {
                            models
                                .iter()
                                .any(|configured| configured == "*" || configured == model)
                        })
                        .unwrap_or(true)
            }
            _ => true,
...
mod tests {
    ...
    fn validates_agent_route_backend_and_policy_references() {
        ...
        let wildcard = Backend {
            name: "all-models".into(),
            kind: BackendKind::Llm {
                provider: "openai".into(),
                models: vec!["*".into()],
                endpoint: None,
                account_type: Some("api-key".into()),
                max_concurrency: None,
                credential_ref: None,
                quota_state: None,
                model_rewrites: std::collections::BTreeMap::new(),
            },
            policies: vec![],
        };
        assert!(wildcard.supports_model(Some("any-current-or-future-model")));
    }
