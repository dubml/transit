...
pub fn generate_schema() -> Result<()> {
	...
	let schemas = vec![
		SchemaDoc {
			...
			file: "config.json",
			schema_json: make::<transit::types::local::LocalConfig>(false)?,
			schema_inline_json: Some(make::<transit::types::local::LocalConfig>(true)?),
		},
		SchemaDoc {
			name: "CEL context",
			mdfile: Some("cel.md"),
		...
		SchemaDoc {
			...
			mdfile: None,
			file: "admin.json",
			schema_json: make_with_contract::<transit::store::StoresDump>(
				false,
	...
	for schema in &schemas {
		let rule_path = format!("{codegen_path}/../../schema/{}", schema.file);
	...
	for schema in schemas {
		...
		let o = if cfg!(target_os = "windows") {
			let cmd_path: String = format!("{codegen_path}/../../tools/schema-to-md.ps1");
			...
			let inline_rule_path = format!("{codegen_path}/../../schema/.inline-{}", schema.file);
			...
			let cmd_path: String = format!("{codegen_path}/../../tools/schema-to-md.sh");
		...
		let mut file = fs_err::File::create(format!("{codegen_path}/../../schema/{mdfile}"))?;
