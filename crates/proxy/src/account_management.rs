...
impl LlmAccounts {
    pub async fn account_models(&self, id: &str) -> Result<Vec<Value>, String> {
        ...
        let mut result = provider_request(&account, Method::GET, path, None).await;
        if result
            .as_ref()
            ...
            && account.summary().can_refresh
        {
            ...
            result = provider_request(&account, Method::GET, path, None).await;
        }
        let payload = result?;
...
async fn provider_request(
    ...
    let base = if account.provider() == "codex" {
        "https://chatgpt.com"
    } else {
...
mod tests {
    ...
    fn model_catalog_only_exposes_names_and_supports_provider_shapes() {
        let catalog = parse_model_catalog(&json!({"models":[{"slug":"gpt-test","display_name":"GPT Test","private_field":"hidden"},{"slug":"gpt-test"}]})).unwrap();
        assert_eq!(
            catalog,
