//! The xDS resource cache and its projection onto transit's configuration model.
//!
//! A transit `Cluster` is a join of an xDS `Cluster` with its
//! `ClusterLoadAssignment`, and a transit `Listener` is a join of an xDS
...
impl AdsState {
    ...
    /// Projects the cached xDS resources onto transit's configuration model and
...
fn listener_snapshot(listener: xds_listener::Listener) -> Result<ListenerSnapshot, XdsError> {
    ...
    for hcm in http_connection_managers(&listener)? {
        merge_listener_security(&mut security, security_from_hcm(&hcm)?);
...
fn security_from_hcm(hcm: &xds_hcm::HttpConnectionManager) -> Result<ListenerSecurity, XdsError> {
    ...
    for filter in &hcm.http_filters {
        let Some(xds_hcm::http_filter::ConfigType::TypedConfig(any)) = filter.config_type.as_ref()
        else {
            continue;
        };
        if filter.name.contains("jwt_authn")
            || any
                .type_url
                .ends_with("extensions.filters.http.jwt_authn.v1.JwtAuthentication")
        {
            let config: xds_jwt::JwtAuthentication = decode_resource(
        ...
        if filter.name.contains("rbac")
            || any
                .type_url
                .ends_with("extensions.filters.http.rbac.v1.RBAC")
        {
            let config: xds_rbac::Rbac = decode_resource(
                "type.googleapis.com/extensions.filters.http.rbac.v1.RBAC",
                any,
            )?;
            security.authorization.push(convert_authorization(&config));
        }
    }
    Ok(security)
}
...
mod tests {
    ...
    use transit_core::{ResourceKey, ResourceKind, RouterIdentity};

    #[test]
    fn identity_metadata_selects_dubbod_grpc_generator() {
        let identity = RouterIdentity {
            pod_name: "transit-abc".into(),
            namespace: "app".into(),
    ...
    fn ads_state_builds_runtime_config_from_lds_rds_cds_and_eds() {
        ...
        let listener = xds_listener::Listener {
            name: "transit.app.svc.cluster.local:80".into(),
        ...
        state.begin_stream(vec!["transit.app.svc.cluster.local:80".into()]);
    ...
    fn route_match_rejects_regex_and_defaults_a_missing_path() {
        ...
        // transit has no regex matcher, so a regex route must be dropped rather than
    ...
    fn security_filters_preserve_required_jwt_and_identity_fields() {
        ...
        assert_eq!(jwt.audiences, ["transit"]);
    ...
    fn valid_sds_secrets() -> (Any, Any) {
        ...
        let certificate =
            RcgenCertificate::from_params(CertificateParams::new(vec!["transit.mesh.test".into()]))
                .unwrap();
