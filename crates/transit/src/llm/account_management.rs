...
pub(crate) struct AccountRequest {
    accounts: Arc<crate::llm::LlmAccounts>,
    id: String,
    result: Option<bool>,
}
...
#[allow(dead_code)]
impl AccountRequest {
    pub(crate) fn new(accounts: Arc<crate::llm::LlmAccounts>, id: String) -> Self {
        Self {
            accounts,
            id,
            result: None,
...
mod tests {
    ...
    async fn health_tracks_stream_completion_error_and_cancellation_per_account() {
        let directory = Directory::new();
        let accounts = Arc::new(LlmAccounts::default());
        accounts
            .configure(&directory.0, "test-management-token-long-enough".into())
            .unwrap();
        accounts.save(account()).await.unwrap();
        let body = AccountRequest::new(accounts.clone(), "account".into())
            .body(Body::from("data: {\"choices\":[]}\n\n"), true);
        hyper::body::to_bytes(body).await.unwrap();
        let body = AccountRequest::new(accounts.clone(), "account".into()).body(
            Body::from("data: {\"error\":{\"message\":\"failed\"}}\n\n"),
            true,
        );
        hyper::body::to_bytes(body).await.unwrap();
        drop(AccountRequest::new(accounts.clone(), "account".into()).body(Body::empty(), false));
        let result = serde_json::to_value(accounts.list()).unwrap();
