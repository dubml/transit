...
pub enum SourceId {
    /// Configuration owned by dynamic API updates.
    Dynamic,
    /// Configuration owned by a standalone instance, including local YAML/JSON fixtures.
    Static,
}

impl SourceId {
    pub const ALL: [SourceId; 2] = [SourceId::Dynamic, SourceId::Static];

    pub fn as_str(&self) -> &'static str {
        match self {
            SourceId::Dynamic => "dynamic",
...
mod tests {
    ...
    fn evicting_a_source_leaves_other_sources_intact() {
        ...
        store.evict_source(SourceId::Dynamic);
        let snapshot = store.snapshot();
    ...
    fn republishing_an_identical_slice_is_a_no_op() {
        ...
        let first = store.apply(SourceId::Dynamic, delta());
        assert!(first.changed);

        let second = store.apply(SourceId::Dynamic, delta());
    ...
    fn readiness_is_false_until_a_source_applies() {
        ...
        store.apply(SourceId::Dynamic, ConfigDelta::default());
        assert!(store.snapshot().ready());
    }
