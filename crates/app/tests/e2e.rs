//! End-to-end tests that drive a real transit proxy (and ui server) over
...
async fn http_route_proxies_to_upstream_and_reports_ui_state() {
    ...
    assert!(text.contains("transit_ready 1"));
    assert!(text.contains("transit_http_route_requests_total{"));
...
async fn oversized_request_body_is_rejected_before_buffering() {
    ...
    // 11 MiB exceeds the 10 MiB default TRANSIT_MAX_BODY_BYTES cap. The proxy
