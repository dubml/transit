...
import (
	...
	hcm "github.com/dubml/xds-api/extensions/filters/v1/network/http_connection_manager"
...
type StatusUpdate struct {
	...
	Addresses  *[]gw.GatewayStatusAddress `json:"addresses,omitempty"`
}
...
func (c Compiler) CompileGateway(ctx krt.HandlerContext, input Resource) *GatewayOutput {
	...
	if err := input.Decode(&gateway); err != nil {
		return nil
	}
	...
	if class.Decode(&gatewayClass) != nil || string(gatewayClass.Spec.ControllerName) != c.ControllerName {
		return nil
	}
	out := &GatewayOutput{Key: gatewayKey(gateway.Namespace, gateway.Name), Resources: map[string]map[string]EncodedResource{}}
	if gatewayClass.Spec.ParametersRef != nil {
		out.Statuses = []StatusUpdate{{Kind: "Gateway", Namespace: gateway.Namespace, Name: gateway.Name, Generation: gateway.Generation, Conditions: []metav1.Condition{
			condition("Accepted", false, "InvalidParameters", "GatewayClass parametersRef is not supported", gateway.Generation),
			condition("Programmed", false, "Invalid", "GatewayClass parametersRef is not supported", gateway.Generation),
		}}}
		out.finishVersion()
		return out
	}
	...
	for _, l := range gateway.Spec.Listeners {
		...
		if l.Protocol != gw.HTTPProtocolType && l.Protocol != gw.HTTPSProtocolType {
			accepted = false
			ls.Conditions = []metav1.Condition{condition("Accepted", false, "UnsupportedProtocol", "This listener requires a supported HTTP transport", gateway.Generation), condition("Programmed", false, "Invalid", "Listener protocol is not supported", gateway.Generation)}
			gatewayStatus.Listeners = append(gatewayStatus.Listeners, ls)
			continue
		}
		transport, certificate, tlsFailure := c.listenerTLS(ctx, gateway, l)
		if tlsFailure != nil {
			accepted = false
			ls.Conditions = []metav1.Condition{
				condition("Accepted", true, "Accepted", "Listener protocol is supported", gateway.Generation),
				condition("ResolvedRefs", false, tlsFailure.reason, tlsFailure.message, gateway.Generation),
				condition("Programmed", false, "Invalid", tlsFailure.message, gateway.Generation),
			}
			gatewayStatus.Listeners = append(gatewayStatus.Listeners, ls)
			continue
		}
		...
		for _, r := range routes {
			...
			if len(hosts) == 0 || !allowed {
				...
				message := "The listener does not allow this Route namespace"
				if len(hosts) == 0 {
					reason = "NoMatchingListenerHostname"
					message = "Route and listener hostnames do not intersect"
				}
				for _, parent := range parents {
					out.Statuses = append(out.Statuses, routeStatus(httpRoute, parent, false, false, reason, message))
				}
				continue
			}
			...
			if err != nil {
				for _, parent := range parents {
					out.Statuses = append(out.Statuses, routeStatus(httpRoute, parent, false, false, "UnsupportedValue", err.Error()))
				}
				continue
			}
			ls.AttachedRoutes++
			...
			for _, parent := range parents {
				out.Statuses = append(out.Statuses, routeStatus(httpRoute, parent, true, true, "Accepted", "Route is included in the published xDS configuration"))
			}
		}
		...
		if transport != nil {
			lds.ApiListener = nil
			lds.FilterChains = []*listener.FilterChain{{Name: name, TransportSocket: transport, Filters: []*listener.Filter{{
				Name: "transit.filters.network.http_connection_manager", ConfigType: &listener.Filter_TypedConfig{TypedConfig: manager},
			}}}}
			out.add(certificate.Name, certificate)
		}
		out.add(lds.Name, lds)
		out.add(routeConfig.Name, routeConfig)
		ls.Conditions = []metav1.Condition{
	...
	reason := "Accepted"
	message := "Gateway configuration is available through xDS"
	if !accepted {
		reason = "ListenersNotValid"
		message = "One or more listeners are not supported; inspect listener conditions"
	}
	accepted = len(out.Resources[ListenerType]) > 0
...
func (out *GatewayOutput) add(name string, message proto.Message) {
	...
	if out.Resources[resource.Value.TypeUrl] == nil {
		out.Resources[resource.Value.TypeUrl] = map[string]EncodedResource{}
	}
...
func (out *GatewayOutput) finishVersion() {
	...
	for _, kind := range slices.Sorted(maps.Keys(out.Resources)) {
		for _, name := range slices.Sorted(maps.Keys(out.Resources[kind])) {
			hash.Write([]byte(kind + "\x00" + name + "\x00" + out.Resources[kind][name].Version))
		}
	}
	statusBytes, _ := json.Marshal(out.Statuses)
...
func matchingParents(r gw.HTTPRoute, gateway gw.Gateway, listener gw.Listener) []gw.ParentReference {
	...
	for _, parent := range r.Spec.ParentRefs {
		if valueOr(parent.Group, "gateway.networking.k8s.io") != "gateway.networking.k8s.io" || valueOr(parent.Kind, "Gateway") != "Gateway" {
			continue
		}
		if valueOr(parent.Namespace, r.Namespace) != gateway.Namespace || string(parent.Name) != gateway.Name {
			continue
		}
...
func (c Compiler) routeNamespaceAllowed(ctx krt.HandlerContext, gateway gw.Gateway, listener gw.Listener, namespace string) bool {
	...
	if len(listener.AllowedRoutes.Kinds) > 0 {
		...
		for _, kind := range listener.AllowedRoutes.Kinds {
			if kind.Kind == "HTTPRoute" && valueOr(kind.Group, "gateway.networking.k8s.io") == "gateway.networking.k8s.io" {
				found = true
			}
		}
		if !found {
	...
	if ns == nil || ns.From == nil || *ns.From == gw.NamespacesFromSame {
		return namespace == gateway.Namespace
	}
	if *ns.From == gw.NamespacesFromAll {
		return true
	}
	if *ns.From != gw.NamespacesFromSelector || ns.Selector == nil {
		return false
	}
	resource := c.Inputs.Get(ctx, "Namespace", "", namespace)
	if resource == nil {
		return false
	}
...
func (c Compiler) referenceAllowed(ctx krt.HandlerContext, sourceKind, sourceNS, targetGroup, targetKind, targetNS, targetName string) bool {
	...
	sourceGroup := "gateway.networking.k8s.io"
	if sourceKind == "TransitService" {
		sourceGroup = TransitGroup
	}
	for _, resource := range c.Inputs.List(ctx, "ReferenceGrant", targetNS) {
		var ref grant.ReferenceGrant
		if resource.Decode(&ref) != nil {
			continue
		}
		from := false
		for _, item := range ref.Spec.From {
			if string(item.Group) == sourceGroup && string(item.Kind) == sourceKind && string(item.Namespace) == sourceNS {
				from = true
			}
		}
		if !from {
		...
		for _, item := range ref.Spec.To {
			if string(item.Group) == targetGroup && string(item.Kind) == targetKind && (item.Name == nil || string(*item.Name) == targetName) {
				return true
			}
		}
	}
...
func intersectHostnames(listener *gw.Hostname, routeHosts []gw.Hostname) []string {
	if len(routeHosts) == 0 {
		return []string{valueOr(listener, "*")}
	}
	result := []string{}
	for _, hostname := range routeHosts {
		host := strings.ToLower(string(hostname))
		bound := strings.ToLower(valueOr(listener, "*"))
		if hostnameMatches(bound, host) {
			result = append(result, host)
		} else if hostnameMatches(host, bound) {
...
func valueOr[T ~string](p *T, fallback string) string {
	if p == nil {
		return fallback
	}
	return string(*p)
...
func deduplicateAgentConfig(config *route.AgentConfig) {
	providers := map[string]*route.AgentProvider{}
	for _, x := range config.Providers {
		providers[x.Name] = x
	}
	config.Providers = nil
	for _, key := range slices.Sorted(maps.Keys(providers)) {
		config.Providers = append(config.Providers, providers[key])
	}
	backends := map[string]*route.AgentBackend{}
	for _, x := range config.Backends {
		backends[x.Name] = x
	}
	config.Backends = nil
	for _, key := range slices.Sorted(maps.Keys(backends)) {
		config.Backends = append(config.Backends, backends[key])
	}
	policies := map[string]*route.AgentPolicy{}
	for _, x := range config.Policies {
		policies[x.Name] = x
	}
	config.Policies = nil
	for _, key := range slices.Sorted(maps.Keys(policies)) {
		config.Policies = append(config.Policies, policies[key])
	}
}
...
func compareRoutes(a, b *route.AgentRoute) int {
	...
	if len(x.Headers) != len(y.Headers) {
		return len(y.Headers) - len(x.Headers)
	}
	return strings.Compare(a.Name, b.Name)
}
