...
type TargetSpec struct {
	...
	BackendRef *struct {
		Name      string `json:"name"`
		Namespace string `json:"namespace,omitempty"`
	} `json:"backendRef,omitempty"`
	Port   int32             `json:"port,omitempty"`
	Path   string            `json:"path,omitempty"`
	Tools  []string          `json:"tools,omitempty"`
	Agent  string            `json:"agent,omitempty"`
	Static *StaticTargetSpec `json:"static,omitempty"`
}
...
func (c Compiler) compileHTTPRoute(ctx krt.HandlerContext, httpRoute gw.HTTPRoute, listener gw.Listener, hosts []string) (*route.AgentConfig, error) {
	...
	for ruleIndex, rule := range httpRoute.Spec.Rules {
		if len(rule.BackendRefs) == 0 {
			return nil, unsupported("rule %d has no backendRefs", ruleIndex)
		}
		...
		for _, backendRef := range rule.BackendRefs {
			...
			if weight < 0 {
				return nil, unsupported("negative backend weight")
			}
			if weight == 0 {
				continue
			}
			if len(backendRef.Filters) > 0 {
				return nil, unsupported("backendRef filters are not supported")
			}
			ns := valueOr(backendRef.Namespace, httpRoute.Namespace)
			kind := valueOr(backendRef.Kind, "Service")
			group := valueOr(backendRef.Group, "")
			...
			var part *route.AgentConfig
			var backendProtocol route.AgentProtocol
			var err error
			switch {
			case group == "" && kind == "Service":
				if backendRef.Port == nil {
					return nil, unsupported("Service backend %s requires a port", backendRef.Name)
				}
				endpoint, err := c.serviceEndpoint(ctx, ns, string(backendRef.Name), int32(*backendRef.Port))
				if err != nil {
					return nil, err
				}
			...
			default:
				return nil, unsupported("unsupported backend kind %s/%s", group, kind)
			}
			if protocol != route.AgentProtocol_AGENT_PROTOCOL_UNSPECIFIED && protocol != backendProtocol {
				return nil, unsupported("a rule cannot mix backend protocols")
			}
			protocol = backendProtocol
			result.Providers = append(result.Providers, part.Providers...)
			result.Backends = append(result.Backends, part.Backends...)
			result.Policies = append(result.Policies, part.Policies...)
			for _, backend := range part.Backends {
				weighted = append(weighted, &route.WeightedBackend{Name: backend.Name, Weight: uint32(weight)})
			}
		}
		if len(weighted) == 0 {
			return nil, unsupported("rule %d has no positive-weight backends", ruleIndex)
		}
		policy, rewrite, err := compileFilters(name+"/filters", rule)
		if err != nil {
			return nil, err
		}
		var policies []string
		...
		for matchIndex, match := range matches {
			if len(match.QueryParams) > 0 {
				return nil, unsupported("query parameter matching is not supported")
			}
			...
			if match.Path != nil {
				...
				case "PathPrefix":
					path.Match = &route.AgentPathMatch_Prefix{Prefix: value}
				case "Exact":
					path.Match = &route.AgentPathMatch_Exact{Exact: value}
				default:
					return nil, unsupported("unsupported path match type")
				}
			}
			if rewrite != nil && path.GetPrefix() == "" {
				return nil, unsupported("ReplacePrefixMatch requires a PathPrefix match")
			}
			var headers []*route.AgentHeaderMatch
			for _, header := range match.Headers {
				if valueOr(header.Type, "Exact") != "Exact" {
					return nil, unsupported("unsupported header match type")
				}
...
func (c Compiler) serviceEndpoint(ctx krt.HandlerContext, namespace, name string, port int32) (string, error) {
	...
	if resource == nil {
		return "", unsupported("Service %s/%s does not exist", namespace, name)
	}
	var service corev1.Service
	if err := resource.Decode(&service); err != nil {
		return "", err
	}
	found := false
	...
	if !found {
		return "", unsupported("Service %s/%s has no TCP port %d", namespace, name, port)
	}
...
func (c Compiler) compileTransitService(ctx krt.HandlerContext, namespace, name string) (*route.AgentConfig, route.AgentProtocol, error) {
	...
	if resource == nil {
		return nil, 0, unsupported("TransitService %s/%s does not exist", namespace, name)
	}
	...
	if service.Spec.A2A != nil {
		count++
	}
	if count != 1 || (service.Spec.LLM != nil && service.Spec.AI != nil) {
		return nil, 0, unsupported("TransitService must select exactly one of llm, ai, mcp or a2a")
	}
	if llm != nil {
		if len(llm.Routes) != 0 {
			return nil, 0, unsupported("custom AI operation routes are not supported by this controller")
		}
		if llm.Provider.OpenAI != nil && llm.Provider.Anthropic != nil {
			return nil, 0, unsupported("AI provider must select exactly one provider")
		}
		provider := &route.AgentProvider{Name: key + "/provider"}
		switch {
		case llm.Provider.Type == "openai" || llm.Provider.OpenAI != nil:
			provider.Kind = route.AgentProviderKind_OPENAI
			if llm.Endpoint == "" {
				llm.Endpoint = "https://api.openai.com/v1"
			}
		case llm.Provider.Type == "anthropic" || llm.Provider.Anthropic != nil:
			provider.Kind = route.AgentProviderKind_ANTHROPIC
			if llm.Endpoint == "" {
				llm.Endpoint = "https://api.anthropic.com"
			}
		default:
			return nil, 0, unsupported("LLM provider must select openai or anthropic")
		}
		if err := validateEndpoint(llm.Endpoint); err != nil {
			return nil, 0, err
		}
		provider.BaseUrl = llm.Endpoint
		...
		for _, h := range llm.Provider.RequestHeaders {
			provider.RequestHeaders = append(provider.RequestHeaders, &route.AgentHeaderValue{Name: h.Name, Value: h.Value})
		}
	...
	if service.Spec.MCP != nil {
		if len(service.Spec.MCP.Targets) == 0 {
			return nil, 0, unsupported("MCP requires at least one target")
		}
		for i, target := range service.Spec.MCP.Targets {
			if target.Static == nil || target.Static.BackendRef == nil {
				return nil, 0, unsupported("MCP target requires static.backendRef and static.port")
			}
			target.BackendRef = target.Static.BackendRef
			target.Port = target.Static.Port
			target.Path = target.Static.Path
			endpoint, err := c.targetEndpoint(ctx, namespace, target)
			if err != nil {
				return nil, 0, err
			}
	...
	endpoint, err := c.targetEndpoint(ctx, namespace, *service.Spec.A2A)
	if err != nil {
		return nil, 0, err
	}
...
func (c Compiler) targetEndpoint(ctx krt.HandlerContext, namespace string, target TargetSpec) (string, error) {
	if target.Endpoint != "" {
		return target.Endpoint, validateEndpoint(target.Endpoint)
	}
	if target.Port < 1 || target.Port > 65535 {
		return "", unsupported("target port must be between 1 and 65535")
	}
	var endpoint string
	if target.BackendRef != nil {
		ns := target.BackendRef.Namespace
		if ns == "" {
			ns = namespace
		}
		if !c.referenceAllowed(ctx, "TransitService", namespace, "", "Service", ns, target.BackendRef.Name) {
			return "", unsupported("TransitService backendRef is not permitted by ReferenceGrant")
		}
		var err error
		endpoint, err = c.serviceEndpoint(ctx, ns, target.BackendRef.Name, target.Port)
		if err != nil {
			return "", err
		}
	} else {
		if target.Host == "" {
			return "", unsupported("target requires endpoint, host, or backendRef")
		}
		endpoint = "http://" + net.JoinHostPort(target.Host, strconv.Itoa(int(target.Port)))
	}
	if target.Path != "" {
		endpoint += "/" + strings.TrimPrefix(target.Path, "/")
	}
...
func validateEndpoint(endpoint string) error {
	...
	if err != nil || parsed.Host == "" || (parsed.Scheme != "http" && parsed.Scheme != "https") || parsed.User != nil || parsed.Fragment != "" {
		return unsupported("endpoint must be an HTTP(S) URL without embedded credentials or fragment")
	}
	return nil
}
...
func (c Compiler) secretReference(ctx krt.HandlerContext, namespace string, ref SecretReference) (*route.SecretKeyReference, error) {
	if ref.Namespace != "" && ref.Namespace != namespace {
		return nil, unsupported("credential Secret must be in the TransitService namespace")
	}
	resource := c.Inputs.Get(ctx, "Secret", namespace, ref.Name)
	if resource == nil {
		return nil, unsupported("credential Secret %s/%s does not exist", namespace, ref.Name)
	}
	value, found, _ := unstructured.NestedString(resource.Object, "data", ref.Key)
	if !found {
		return nil, unsupported("credential Secret %s/%s has no requested key", namespace, ref.Name)
	}
	decoded, err := base64.StdEncoding.DecodeString(value)
	if err != nil || len(decoded) == 0 {
		return nil, unsupported("credential Secret contains an invalid or empty value")
	}
...
func (c Compiler) compilePolicy(ctx krt.HandlerContext, namespace, name string, spec PolicySpec) (*route.AgentPolicy, error) {
	...
	var err error
	if spec.Timeout != "" {
		p.Timeout, err = positiveDuration(spec.Timeout)
		if err != nil {
			return nil, err
		}
	}
	if spec.Auth != nil {
		ref, err := c.secretReference(ctx, namespace, spec.Auth.SecretRef)
		if err != nil {
			return nil, err
		}
		if spec.Auth.Header == "" {
			return nil, unsupported("auth.header is required")
		}
		p.Auth = &route.ClientAuthPolicy{Header: spec.Auth.Header, SecretRef: ref}
	}
	if spec.Retry != nil {
		p.Retry = &route.AgentRetryPolicy{Attempts: spec.Retry.Attempts, StatusCodes: spec.Retry.StatusCodes}
	}
	if spec.RateLimit != nil {
		window, err := positiveDuration(spec.RateLimit.Window)
		if err != nil {
			return nil, err
		}
		key, err := rateLimitKey(spec.RateLimit.Key, spec.RateLimit.Header)
		if err != nil {
			return nil, err
		}
		if spec.RateLimit.Requests == 0 {
			return nil, unsupported("rateLimit.requests must be positive")
		}
	...
	if spec.TokenLimit != nil {
		window, err := positiveDuration(spec.TokenLimit.Window)
		if err != nil {
			return nil, err
		}
		key, err := rateLimitKey(spec.TokenLimit.Key, spec.TokenLimit.Header)
		if err != nil {
			return nil, err
		}
		if spec.TokenLimit.Tokens == 0 {
			return nil, unsupported("tokenLimit.tokens must be positive")
		}
	...
	if spec.RequestHeaders != nil {
		p.RequestHeaders = headerTransform(spec.RequestHeaders.Add, spec.RequestHeaders.Remove)
	}
	if spec.ResponseHeaders != nil {
		p.ResponseHeaders = headerTransform(spec.ResponseHeaders.Add, spec.ResponseHeaders.Remove)
	}
	return p, nil
}
func headerTransform(headers []Header, remove []string) *route.AgentHeaderTransform {
	x := &route.AgentHeaderTransform{Remove: remove}
	for _, h := range headers {
		x.Add = append(x.Add, &route.AgentHeaderValue{Name: h.Name, Value: h.Value})
	}
	return x
}
func rateLimitKey(key, header string) (route.AgentRateLimitKey, error) {
	switch strings.ToLower(key) {
	case "", "route", "rate_limit_key_unspecified":
		return route.AgentRateLimitKey_ROUTE, nil
	case "backend":
		return route.AgentRateLimitKey_BACKEND, nil
	case "header":
		if header != "" {
			return route.AgentRateLimitKey_HEADER, nil
		}
	}
...
func positiveDuration(value string) (*durationpb.Duration, error) {
	...
	if err != nil || d <= 0 {
		return nil, unsupported("duration must be positive: %q", value)
	}
	return durationpb.New(d), nil
}

func compileFilters(name string, rule gw.HTTPRouteRule) (*route.AgentPolicy, *route.PathRewrite, error) {
	p := &route.AgentPolicy{Name: name}
	used := false
	var rewrite *route.PathRewrite
	for _, filter := range rule.Filters {
		switch filter.Type {
		case gw.HTTPRouteFilterRequestHeaderModifier, gw.HTTPRouteFilterResponseHeaderModifier:
			headers := filter.RequestHeaderModifier
			if filter.Type == gw.HTTPRouteFilterResponseHeaderModifier {
				headers = filter.ResponseHeaderModifier
			}
			if headers == nil {
				return nil, nil, unsupported("header modifier has no configuration")
			}
			x := &route.AgentHeaderTransform{Remove: headers.Remove}
			for _, h := range headers.Set {
				x.Remove = append(x.Remove, string(h.Name))
				x.Add = append(x.Add, &route.AgentHeaderValue{Name: string(h.Name), Value: h.Value})
			}
		...
		case gw.HTTPRouteFilterURLRewrite:
			if rewrite != nil || filter.URLRewrite == nil || filter.URLRewrite.Hostname != nil || filter.URLRewrite.Path == nil || filter.URLRewrite.Path.Type != gw.PrefixMatchHTTPPathModifier || filter.URLRewrite.Path.ReplacePrefixMatch == nil {
				return nil, nil, unsupported("URLRewrite supports one ReplacePrefixMatch path modifier")
			}
			rewrite = &route.PathRewrite{ReplacePrefixMatch: *filter.URLRewrite.Path.ReplacePrefixMatch}
		default:
			return nil, nil, unsupported("unsupported HTTPRoute filter %s", filter.Type)
		}
	}
	if rule.Timeouts != nil && rule.Timeouts.Request != nil && string(*rule.Timeouts.Request) != "0s" {
		var err error
		p.Timeout, err = positiveDuration(string(*rule.Timeouts.Request))
		if err != nil {
			return nil, nil, err
		}
		used = true
